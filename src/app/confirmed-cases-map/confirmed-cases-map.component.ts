...
export class ConfirmedCasesMapComponent implements AfterViewInit {
  ...
  public componentDidMount() {
      ...
      fetch('../../assets/Data/time_series_19-covid-Confirmed.csv')
          ...
          .then((data) => this.onConfirmedDataLoaded(data));
  }

  public onConfirmedDataLoaded(csvData: string) {
  ...
  public onDeathDataLoaded(csvData: string) {
    const csvLines = csvData.split('\n');
    const lat: number[] = [];
    const lon: number[] = [];
    const val: number[] = [];
  ...
  public onRecoveredDataLoaded(csvData: string) {
    const csvLines = csvData.split('\n');
    const lat: number[] = [];
    const lon: number[] = [];
    const val: number[] = [];
